...
# ── 路径 ──
# 脚本放在 theme-lab 下，仓库根目录是它的上一级
$Root = Split-Path -Parent $PSScriptRoot
$LyricServer = Join-Path $Root 'lyric-overlay\src\server.mjs'
$UiServer = Join-Path $Root 'theme-lab\serve.mjs'
$UiRoot = Join-Path $Root 'theme-lab'

foreach ($f in @($LyricServer, $UiServer)) {
  if (-not (Test-Path $f)) {
